...
def jw_method(clauses, branches):
    while True:
    ...
    for clause in clauses:
        for literal in clause:
            if literal in d:
                d[literal]=d[literal]+pow(2,len(clause))
            else:
                d[literal]=pow(2,len(clause))
    ...
    result1, branches1 = jw_method(clauses_copy1, branches)
    if result1 == "Satisfiable":
        return result1, branches1

    rule1(clauses_copy2, compliment(chosen_literal))
    result2, branches2 = jw_method(clauses_copy2, branches)
