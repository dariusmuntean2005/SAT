...
def moms_choose_literal(clauses):
    if not clauses:
        return None

    min_size = min(len(clause) for clause in clauses if len(clause) > 0)
    ...
    return max(d, key=lambda lit: d[lit])


def moms_method(clauses,branches):
    while True:
    ...
    chosen_literal = moms_choose_literal(clauses)
    ...
    result1, branches1 = moms_method(clauses_copy1, branches)
    if result1 == "Satisfiable":
        return result1, branches1

    rule1(clauses_copy2, compliment(chosen_literal))
    result2, branches2 = moms_method(clauses_copy2, branches)
